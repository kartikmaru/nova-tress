...
function StatPill({ target, suffix = "+", label, started }) {
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <span className="font-playfair text-4xl lg:text-5xl font-bold text-white leading-none">
        {value}
        <span className="text-accent">{suffix}</span>
      </span>
      <span className="font-poppins text-xs uppercase tracking-widest text-white/50">
        {label}
      </span>
    </div>
  );
    ...
    >
      ...
      <div className="relative z-20 flex-1 flex flex-col justify-center px-6 lg:px-20 pt-28 pb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link
            href="/services"
            className="inline-block font-poppins text-sm font-semibold uppercase tracking-widest
              px-8 py-3.5 rounded border border-white/30 text-white
              hover:border-accent hover:text-accent transition-all duration-300 text-center"
          >
            Explore Services
          </Link>
      ...
      <div className="relative z-20 px-6 lg:px-20 py-10 border-t border-white/10">
        <div className="flex flex-row flex-wrap gap-10 sm:gap-20">
